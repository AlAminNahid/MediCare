...
import { getFieldError } from "@/validation/helpers";

// Live checklist UI only — the source of truth for validation is passwordSchema
// in validation/auth.schema.ts; these mirror the same 4 rules for the checkmarks.
const pwRules = [
...
export default function RegisterPage() {
  ...
  const blurValidate = (field: keyof typeof errors) => {
    const msg = getFieldError(registerSchema, field, form[field]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setErrors({
        fullName: getFieldError(registerSchema, "fullName", form.fullName),
        email: getFieldError(registerSchema, "email", form.email),
        phoneNumber: getFieldError(registerSchema, "phoneNumber", form.phoneNumber),
        password: getFieldError(registerSchema, "password", form.password),
      });
      return;
    }
      ...
      <div className="hidden w-1/2 flex-col justify-between bg-slate-900 p-12 lg:flex">
        ...
        <div>
          ...
          <p className="mt-4 text-slate-400">
            Manage your chamber, patients, and appointments all from one place.
          </p>
      ...
      <div className="flex flex-1 items-center justify-center bg-slate-50 px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            <Link
              href="/"
              className="mb-6 flex w-fit items-center gap-2 lg:hidden transition-opacity hover:opacity-70"
            >
                ...
                <ul className="mt-2.5 space-y-1.5 rounded-lg border border-slate-200 bg-white p-3">
                  {pwRules.map(({ label, test }) => {
                    ...
                    return (
                      <li
                        key={label}
                        className={`flex items-center gap-2 text-xs ${passed ? "text-green-600" : "text-slate-400"}`}
                      >
                        {passed ? (
