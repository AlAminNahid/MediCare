...
export default function DoctorChambersPage() {
  ...
  const handleCreate = async () => {
    setSaving(true);
    try {
      const added = (await api.doctor.createChamber(
        toPayload(form),
      )) as Chamber;
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      const updated = (await api.doctor.updateChamber(
        editing.chamberId,
        toPayload(editForm),
      )) as Chamber;
      setChambers((prev) =>
        prev.map((c) => (c.chamberId === editing.chamberId ? updated : c)),
      );
      setEditing(null);
    } catch (err: unknown) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Manage the locations and schedules you sit at
            </p>
          </div>
        </div>
        <button
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {ALL_DAYS.map((d) => (
                        <span
                          key={d}
                          className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${days.includes(d) ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-400"}`}
                        >
                          {d}
                        </span>
                      ))}
            ...
            <div className="space-y-5 p-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. City Health Point"
              ...
              <div>
                ...
                <input
                  ...
                  value={form.visitFee}
                  onChange={(e) =>
                    setForm({ ...form, visitFee: e.target.value })
                  }
                  className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
            ...
            <div className="space-y-5 p-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, visitFee: e.target.value })
                  }
                  className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ALL_DAYS.map((d) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        toggleDay(d, editForm.days, (days) =>
