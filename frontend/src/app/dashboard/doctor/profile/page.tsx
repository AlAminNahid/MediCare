...
export default function DoctorProfilePage() {
  const [form, setForm] = useState({ fullName: '', email: '', phoneNumber: '', specialization: '', visitFee: 0, password: '' });
  ...
  useEffect(() => {
    api.doctor.getProfile().then((d: any) => {
      setForm({ fullName: d.fullName, email: d.email || '', phoneNumber: d.phoneNumber, specialization: d.specialization, visitFee: d.visitFee, password: '' });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      await api.doctor.updateProfile(form);
      setSuccess(true);
        ...
        <form onSubmit={handleSave} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
          <div className="flex items-center gap-4 pb-4 border-b border-slate-100">
            ...
            <div>
              ...
              <span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">{form.specialization}</span>
            </div>
          </div>
