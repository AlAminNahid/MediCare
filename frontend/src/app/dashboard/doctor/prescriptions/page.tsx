...
export default function DoctorPrescriptionsPage() {
  ...
  const [suggestions, setSuggestions] = useState<
    Record<number, MedicineSuggestion[]>
  >({});
  ...
  const searchMedicine = useCallback((idx: number, q: string) => {
    ...
    searchTimers.current[idx] = setTimeout(async () => {
      const results = (await api.doctor.searchMedicines(
        q,
  ...
  const addTest = () => {
    ...
    if (t && !form.tests.includes(t))
      setForm({ ...form, tests: [...form.tests, t] });
    setTestInput("");
  };
  ...
  const handleSave = async () => {
    if (
      (!editingId && !form.patientId) ||
      form.medicines.some((m) => !m.medicineName)
    )
      return;
    setSaving(true);
    try {
  ...
  const selectedChamber = chambers.find(
    (c) => c.chamberId === Number(form.chamberId),
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Issue and view digital prescriptions
            </p>
          </div>
        </div>
        <button
          ...
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 sticky top-0 bg-white z-10">
              <h2 className="font-semibold text-slate-900">
                {editingId !== null ? "Edit Prescription" : "New Prescription"}
              </h2>
              <button
                onClick={closeModal}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, chamberId: e.target.value })
                    }
                    className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                  >
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-12 gap-2 px-1">
                    <p className="col-span-5 text-xs font-medium text-slate-400">
                      Name
                    </p>
                    <p className="col-span-3 text-xs font-medium text-slate-400">
                      Dosage
                    </p>
                    <p className="col-span-3 text-xs font-medium text-slate-400">
                      Duration
                      ...
                      <div className="col-span-5 relative">
                        <div className="relative">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateMed(idx, "medicineName", e.target.value)
                            }
                            onBlur={() =>
                              setTimeout(
                                () =>
                            ...
                            <div className="absolute top-full left-0 right-0 z-20 mt-1 rounded-lg border border-slate-200 bg-white shadow-lg overflow-hidden">
                              {suggestions[idx].map((sug) => (
                                <button
                                  key={sug.medicineId}
                                  type="button"
                                  onMouseDown={() => pickSuggestion(idx, sug)}
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Special instructions, dietary advice, follow-up..."
                  value={form.notes}
              ...
              {selectedChamber && (
                <div className="rounded-lg border border-indigo-100 bg-indigo-50 px-4 py-3 text-xs text-indigo-700">
                  ...
                  <p className="mt-0.5">
                    {selectedChamber.name} · {selectedChamber.address}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4 sticky bottom-0 bg-white">
              ...
              <button
                ...
                disabled={
                  saving ||
                  (editingId === null && !form.patientId) ||
                  form.medicines.every((m) => !m.medicineName)
                }
