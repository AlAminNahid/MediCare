...
export default function BookAppointmentPage() {
  ...
  const selectedChamber =
    chambers.find((c) => c.chamberId === chamberId) || null;
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Find a doctor by location, then pick a chamber
          </p>
        </div>
      </div>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-green-600">
              You can view your serial number in My Appointments.
            </p>
          </div>
          <button
      ...
      <div className="mb-5">
        ...
        {locationQuery && !searching && (
          <p className="mt-1.5 text-xs text-slate-500">
            {doctors.length > 0
              ? `${doctors.length} doctor${doctors.length !== 1 ? "s" : ""} found near "${locationQuery}"`
              : `No doctors found near "${locationQuery}"`}
          </p>
        )}
      </div>
                      ...
                      {Array.isArray(doc.degrees) && doc.degrees.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1">
                          {doc.degrees.map((d) => (
                            <span
                              key={d.degree}
                              className="flex items-center gap-0.5 rounded-full bg-indigo-50 px-1.5 py-0.5 text-[10px] font-medium text-indigo-600"
                            >
          ...
          >
            {!selectedDoctor ? (
              <div className="py-10 text-center text-slate-400">
                Select a doctor to see their chambers
              </div>
            ) : (
              <>
                {error && (
                ...
                <div>
                  ...
                  {chambersLoading ? (
                    ...
                    <div className="rounded-lg border border-dashed border-slate-300 py-6 text-center text-sm text-slate-400">
                      This doctor has no chambers yet
                    </div>
                  ) : (
                  ...
                  <div className="flex items-center gap-3 rounded-lg bg-indigo-50 p-4">
                    ...
                    <div>
                      <p className="text-sm font-medium text-indigo-900">
                        {selectedDoctor.fullName} · {selectedChamber.name}
                      </p>
                      <p className="text-xs text-indigo-600">
                        You'll receive the next available serial number for this
                        date
                      </p>
                    </div>
                  </div>
