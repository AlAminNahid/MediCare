...
export default function AdminSettingsPage() {
  ...
  const handleProfileSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMsg((err as Error).message || "Failed to update profile", true);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePasswordSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMsg((err as Error).message || "Failed to update password", true);
    } finally {
      setSaving(false);
    }
  };
  ...
  const field = (label: string, value: string) => (
    <div key={label}>
      ...
      <p className="text-sm text-slate-800">
        {value || <span className="text-slate-400 italic">Not added</span>}
      </p>
    </div>
  );
