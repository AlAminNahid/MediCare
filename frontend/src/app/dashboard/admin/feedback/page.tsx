...
export default function AdminFeedbackPage() {
  ...
  useEffect(() => {
    api.admin
      .getFeedbacks()
      .then((d) => setFeedbacks(d as Feedback[]))
  ...
  const markReviewed = async (id: number) => {
    ...
    try {
      ...
      setFeedbacks((prev) =>
        prev.map((f) =>
          f.feedbackId === id ? { ...f, status: "reviewed" } : f,
        ),
      );
    } finally {
      setMarking(null);
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            Messages submitted by doctors and patients
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {feedbacks.map((fb) => (
            <div
              key={fb.feedbackId}
              className={`rounded-xl border bg-white shadow-sm overflow-hidden transition ${fb.status === "reviewed" ? "border-slate-100 opacity-70" : "border-slate-200"}`}
            >
              <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-3">
                <div className="flex items-center gap-2.5">
                  <div
                    className={`flex h-7 w-7 items-center justify-center rounded-full ${fb.senderRole === "doctor" ? "bg-blue-100" : "bg-green-100"}`}
                  >
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${fb.senderRole === "doctor" ? "bg-blue-50 text-blue-700" : "bg-green-50 text-green-700"}`}
                  >
                    {fb.senderRole}
                  </span>
                </div>
