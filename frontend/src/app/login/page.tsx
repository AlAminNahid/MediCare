...
export default function LoginPage() {
  ...
  const blurValidate = (field: "email" | "password") => {
    const msg = getFieldError(loginSchema, field, form[field]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setErrors({
        email: getFieldError(loginSchema, "email", form.email),
        password: getFieldError(loginSchema, "password", form.password),
      });
      return;
    }
      ...
      <div className="hidden w-1/2 flex-col justify-between bg-slate-900 p-12 lg:flex">
        ...
        <div>
          <blockquote className="text-2xl font-medium leading-snug text-white">
            "Streamline your chamber. Empower your team. Deliver exceptional
            patient care."
          </blockquote>
          <p className="mt-4 text-slate-400">
            The all-in-one medical chamber management platform.
          </p>
        </div>
